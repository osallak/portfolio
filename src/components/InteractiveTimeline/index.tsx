...
const TimelineEvent = ({ event, isActive, onClick }) => {
  ...
  const eventElements = useMemo(() => {
    return {
      ...
      dateTag: (
        <div className="absolute -top-4 left-1/2 transform -translate-x-1/2 bg-[#8c1df3] text-white px-3 py-1 rounded-full text-sm font-semibold shadow-lg">
          {event.date}
        </div>
      ),
      icon: (
        <div
          className={`w-12 h-12 mx-auto mb-4 rounded-full flex items-center justify-center bg-[#2e2e2e] transition-all duration-300`}
        >
  ...
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -20 }}
      className={`relative p-5 sm:p-6 mb-8 rounded-lg border ${
        isActive
    ...
    >
      ...
      <p className="text-[#ababab] text-center">{event.description}</p>
    </motion.div>
  );
};
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Timeline */}
        <div className="relative mt-16 px-2 sm:px-0">
          ...
          {hasMoreEvents && (
            <div className="flex justify-center mt-8">
              <motion.button
                ...
                className="px-6 py-3 rounded-full bg-[#2e2e2e] text-white/80 hover:text-white hover:bg-gradient-to-r hover:from-[#8c1df3] hover:to-[#621aaf] transition-all duration-300 font-medium flex items-center gap-2"
                whileHover={{ scale: 1.05 }}
                whileTap={{ scale: 0.95 }}
                layout
              >
        ...
        <motion.div className="px-2 sm:px-0 mt-8" variants={futureVariants}>
          <div className="p-6 border border-[#2e2e2e] bg-[#1a1a1a]/50 backdrop-blur-sm rounded-lg">
            ...
            <p className="text-[#ababab] text-center max-w-3xl mx-auto">
              As I continue advancing in my software engineering journey, I'm
              particularly focused on mastering distributed systems,
              ...
              programming, and DevOps, I'm now exploring areas like Kubernetes,
              infrastructure as code, and Web3 technologies. My goal is to
              create scalable, high-impact solutions that combine technical
              excellence with exceptional user experiences, while continuing to
              contribute to meaningful projects and expand my expertise across
